...
## Hetzner Cloud DNS Provider (Fork)

This fork adds support for the **Hetzner Cloud DNS API** (`api.hetzner.cloud`) via the `hetznerhcloud` provider.

```bash
HCLOUD_TOKEN=your-token lego --dns hetznerhcloud -d example.com run
```

See `providers/dns/hetznerhcloud/` for provider documentation.
...
<table><tr>
  ...
  <td><a href="https://go-acme.github.io/lego/dns/hetznerhcloud/">Hetzner Cloud</a></td>
