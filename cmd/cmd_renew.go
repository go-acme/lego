...
func renew(_ context.Context, cmd *cli.Command) error {
	...
	// CSR
	if cmd.IsSet(flgCSR) {
		return renewForCSR(cmd, account, keyType, certsStorage, bundle, meta)
	}

	// Domains
	return renewForDomains(cmd, account, keyType, certsStorage, bundle, meta)
}

func renewForDomains(cmd *cli.Command, account *Account, keyType certcrypto.KeyType, certsStorage *CertificatesStorage, bundle bool, meta map[string]string) error {
	...
	if !cmd.Bool(flgARIDisable) {
		...
		ariRenewalTime = getARIRenewalTime(cmd, cert, domain, client)
	...
	if ariRenewalTime == nil && !needRenewal(cert, domain, cmd.Int(flgDays)) &&
...
func renewForCSR(cmd *cli.Command, account *Account, keyType certcrypto.KeyType, certsStorage *CertificatesStorage, bundle bool, meta map[string]string) error {
...
func getARIRenewalTime(cmd *cli.Command, cert *x509.Certificate, domain string, client *lego.Client) *time.Time {
	if cert.IsCA {
		log.Fatalf("[%s] Certificate bundle starts with a CA certificate", domain)
	}
