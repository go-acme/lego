---
title: "Poweradmin"
date: 2019-03-03T16:39:46+01:00
draft: false
slug: poweradmin
dnsprovider:
  since:    "v5.2.0"
  code:     "poweradmin"
  url:      "https://www.poweradmin.org/"
---

<!-- THIS DOCUMENTATION IS AUTO-GENERATED. PLEASE DO NOT EDIT. -->
<!-- providers/dns/poweradmin/poweradmin.toml -->
<!-- THIS DOCUMENTATION IS AUTO-GENERATED. PLEASE DO NOT EDIT. -->


Configuration for [Poweradmin](https://www.poweradmin.org/).


<!--more-->

- Code: `poweradmin`
- Since: v5.2.0


Here is an example bash command using the Poweradmin provider:

```bash
POWERADMIN_BASE_URL="https://example.org" \
POWERADMIN_API_KEY="xxx" \
lego --dns poweradmin -d '*.example.com' -d example.com run
```




## Credentials

| Environment Variable Name | Description |
|-----------------------|-------------|
| `POWERADMIN_API_KEY` | API key |
| `POWERADMIN_BASE_URL` | Base URL |

The environment variable names can be suffixed by `_FILE` to reference a file instead of a value.
More information [here]({{% ref "dns#configuration-and-credentials" %}}).


## Additional Configuration

| Environment Variable Name | Description |
|--------------------------------|-------------|
| `POWERADMIN_HTTP_TIMEOUT` | API request timeout in seconds (Default: 30) |
| `POWERADMIN_POLLING_INTERVAL` | Time between DNS propagation check in seconds (Default: 2) |
| `POWERADMIN_PROPAGATION_TIMEOUT` | Maximum waiting time for DNS propagation in seconds (Default: 60) |
| `POWERADMIN_TTL` | The TTL of the TXT record used for the DNS challenge in seconds (Default: 120) |

The environment variable names can be suffixed by `_FILE` to reference a file instead of a value.
More information [here]({{% ref "dns#configuration-and-credentials" %}}).




## More information

- [API documentation](https://docs.poweradmin.org/configuration/api/)

<!-- THIS DOCUMENTATION IS AUTO-GENERATED. PLEASE DO NOT EDIT. -->
<!-- providers/dns/poweradmin/poweradmin.toml -->
<!-- THIS DOCUMENTATION IS AUTO-GENERATED. PLEASE DO NOT EDIT. -->
