...
func TestPresent_httpStorage(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			config := clientmock.NewBuilder(func(server *httptest.Server) (*Config, error) {
				...
				// Fetch
				Route("GET /example.com", clientmock.Noop().WithStatusCode(http.StatusNotFound)).
				// Put
				Route("POST /example.com", clientmock.Noop().WithStatusCode(test.StatusCode)).
				Build(t)
