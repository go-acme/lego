// Package dnsservices implements a DNS provider for solving the DNS-01 challenge using DNS.services.
package dnsservices
...
import (
	...
	"github.com/go-acme/lego/v5/providers/dns/dnsservices/internal"
	"github.com/go-acme/lego/v5/providers/dns/internal/clientdebug"
)

// Environment variables names.
const (
	envNamespace = "DNSSERVICES_"
...
// NewDNSProvider returns a DNSProvider instance configured for DNS.services.
func NewDNSProvider() (*DNSProvider, error) {
	values, err := env.Get(EnvUsername, EnvPassword)
	if err != nil {
		return nil, fmt.Errorf("dnsservices: %w", err)
	}
...
// NewDNSProviderConfig return a DNSProvider instance configured for DNS.services.
func NewDNSProviderConfig(config *Config) (*DNSProvider, error) {
	if config == nil {
		return nil, errors.New("dnsservices: the configuration of the DNS provider is nil")
	}
...
func (d *DNSProvider) Present(ctx context.Context, domain, token, keyAuth string) error {
	...
	if err != nil {
		return fmt.Errorf("dnsservices: could not find zone for domain %q: %w", domain, err)
	}

	subDomain, err := dns01.ExtractSubDomain(info.EffectiveFQDN, authZone)
	if err != nil {
		return fmt.Errorf("dnsservices: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("dnsservices: add record: %w", err)
	}
...
func (d *DNSProvider) CleanUp(ctx context.Context, domain, token, keyAuth string) error {
	...
	if !ok {
		return fmt.Errorf("dnsservices: unknown zone for '%s' '%s'", info.EffectiveFQDN, token)
	}
	...
	if err != nil {
		return fmt.Errorf("dnsservices: remove record: %w", err)
	}
