...
func TestClient_Publish(t *testing.T) {
	client := mockBuilder().
		Route("PUT /Zone/example.com", clientmock.ResponseFromFixture("publish.json"),
			clientmock.CheckRequestJSONBody(`{"publish":true,"notes":"my message"}`)).
		Build(t)
...
func TestClient_AddTXTRecord(t *testing.T) {
	client := mockBuilder().
		Route("POST /TXTRecord/example.com/example.com.", clientmock.ResponseFromFixture("create-txt-record.json"),
			clientmock.CheckRequestJSONBody(`{"rdata":{"txtdata":"txt"},"ttl":"120"}`)).
		Build(t)
