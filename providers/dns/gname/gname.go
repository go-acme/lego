// Package gname implements a DNS provider for solving the DNS-01 challenge using GName.
package gname
...
import (
	...
	"github.com/go-acme/lego/v5/providers/dns/gname/internal"
...
// NewDNSProvider returns a DNSProvider instance configured for GName.
...
// NewDNSProviderConfig return a DNSProvider instance configured for GName.
func NewDNSProviderConfig(config *Config) (*DNSProvider, error) {
	if config == nil {
		return nil, errors.New("gname: the configuration of the DNS provider is nil")
	}

	client, err := internal.NewClient(config.AppID, config.AppKey)
	if err != nil {
		return nil, fmt.Errorf("gname: %w", err)
	}
...
func (d *DNSProvider) Present(ctx context.Context, domain, token, keyAuth string) error {
	...
	if err != nil {
		return fmt.Errorf("gname: add record: %w", err)
	}
...
func (d *DNSProvider) CleanUp(ctx context.Context, domain, token, keyAuth string) error {
	...
	if !ok {
		return fmt.Errorf("gname: unknown record ID for '%s' '%s'", info.EffectiveFQDN, token)
	}
	...
	if err != nil {
		return fmt.Errorf("gname: delete record: %w", err)
	}
