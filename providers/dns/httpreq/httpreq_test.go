...
func TestNewDNSProvider_Present(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			builder: mockBuilder("").
				Route("/present",
					clientmock.RawStringResponse("lego"),
					clientmock.CheckRequestJSONBody(`{"fqdn":"_acme-challenge.domain.","value":"LHDhK3oGRvkiefQnx7OOczTY5Tic_xZ6HcMOc_gmtoM"}`)),
		},
		{
		...
		},
		{
			...
			builder: mockBuilder("RAW").
				Route("/present",
					clientmock.RawStringResponse("lego"),
					clientmock.CheckRequestBody(`{"domain":"domain","token":"token","keyAuth":"key"}`)),
		},
		{
			desc:          "error raw mode",
			builder:       mockBuilder("RAW"),
...
func mockBuilder(mode string) *clientmock.Builder[*DNSProvider] {
...
func mockBuilderWithPathPrefix(mode, prefix string) *clientmock.Builder[*DNSProvider] {
	return clientmock.NewBuilder(
		func(server *httptest.Server) (*DNSProvider, error) {
			config := NewDefaultConfig()
...
func mockBuilderWithBasicAuth(username, password string) *clientmock.Builder[*DNSProvider] {
	return clientmock.NewBuilder(
		func(server *httptest.Server) (*DNSProvider, error) {
			config := NewDefaultConfig()
			config.Endpoint, _ = url.Parse(server.URL)
		...
		clientmock.CheckHeader().WithBasicAuth("user", "secret"))
}
