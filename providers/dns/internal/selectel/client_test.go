...
func TestClient_ListRecords(t *testing.T) {
	client := clientmock.NewBuilder[*Client](setupClient,
		...
		Route("GET /123/records/", clientmock.ResponseFromFixture("list_records.json")).
		Build(t)
...
func TestClient_ListRecords_error(t *testing.T) {
	client := clientmock.NewBuilder[*Client](setupClient,
		...
		Route("GET /123/records/",
			clientmock.ResponseFromFixture("error.json").WithStatusCode(http.StatusUnauthorized)).
		Build(t)
...
func TestClient_GetDomainByName(t *testing.T) {
	client := clientmock.NewBuilder[*Client](setupClient,
		...
		Route("GET /sub.sub.example.org",
			clientmock.Noop().WithStatusCode(http.StatusNotFound)).
		Route("GET /sub.example.org",
			clientmock.Noop().WithStatusCode(http.StatusNotFound)).
		Route("GET /example.org",
			clientmock.ResponseFromFixture("domains.json")).
		Build(t)
