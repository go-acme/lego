...
func TestClient_ListRecords(t *testing.T) {
	client := servermock2.NewBuilder[*Client](setupClient,
		...
		Route("GET /123/records/", servermock2.ResponseFromFixture("list_records.json")).
		Build(t)
...
func TestClient_ListRecords_error(t *testing.T) {
	client := servermock2.NewBuilder[*Client](setupClient,
		...
		Route("GET /123/records/",
			servermock2.ResponseFromFixture("error.json").WithStatusCode(http.StatusUnauthorized)).
		Build(t)
...
func TestClient_GetDomainByName(t *testing.T) {
	client := servermock2.NewBuilder[*Client](setupClient,
		...
		Route("GET /sub.sub.example.org",
			servermock2.Noop().WithStatusCode(http.StatusNotFound)).
