// Package ionoscloud implements a DNS provider for solving the DNS-01 challenge using IONOS Cloud Public DNS.
package ionoscloud
...
import (
	...
	internal "github.com/go-acme/lego/v4/providers/dns/internal/ionoscloud"
)
...
func NewDefaultConfig() *Config {
	return &Config{
		TTL:                env.GetOrDefaultInt(EnvTTL, internal.MinTTL),
...
// Credentials must be passed in the environment variables: IONOSCLOUD_TOKEN.
...
func NewDNSProviderConfig(config *Config) (*DNSProvider, error) {
	...
	provider, err := internal.NewDNSProviderConfig(config, "")
	if err != nil {
		return nil, fmt.Errorf("ionoscloud: %w", err)
	}
