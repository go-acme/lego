...
`
	...
	client := mockBuilder().
		Route("POST /", servermock2.RawStringResponse(responseBody),
			servermock2.CheckHeader().WithContentType("text/xml; charset=utf-8")).
		Build(t)
...
`,
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			client := mockBuilder().
				Route("POST /",
					servermock2.RawStringResponse(test.responseBody).
...
`
	...
	client := mockBuilder().
		Route("GET /", servermock2.RawStringResponse(responseBody)).
		Build(t)
...
`,
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			client := mockBuilder().
				Route("GET /",
					servermock2.RawStringResponse(test.responseBody).WithStatusCode(test.statusCode)).
				Build(t)
