...
import (
	...
	"github.com/go-acme/lego/v5/providers/dns/noip/internal"
)
...
func (d *DNSProvider) Present(ctx context.Context, domain, token, keyAuth string) error {
	...
	err = d.client.CreateRData(ctx, dns01.UnFqdn(authZone), subDomain, "TXT", data)
	if err != nil {
		return fmt.Errorf("noip: %w", err)
	}
...
func (d *DNSProvider) CleanUp(ctx context.Context, domain, token, keyAuth string) error {
	...
	err = d.client.DeleteRDataByLabel(ctx, dns01.UnFqdn(authZone), subDomain, "TXT", token)
	if err != nil {
		return fmt.Errorf("noip: %w", err)
	}
