...
func TestNewDNSProviderConfig(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expected: "opusdns: credentials missing",
		},
	}
...
func TestDNSProvider_Present(t *testing.T) {
	provider := mockBuilder().
		Route("PATCH /v1/dns/example.com/records",
			servermock.Noop(),
			servermock.CheckRequestJSONBodyFromInternal("patch_records-upsert-request.json"),
		).
		Build(t)
...
func TestDNSProvider_CleanUp(t *testing.T) {
	provider := mockBuilder().
		Route("PATCH /v1/dns/example.com/records",
			servermock.Noop(),
			servermock.CheckRequestJSONBodyFromInternal("patch_records-remove-request.json"),
		).
		Build(t)
