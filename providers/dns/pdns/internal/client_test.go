...
func mockBuilder() *clientmock.Builder[*Client] {
	return clientmock.NewBuilder[*Client](
		...
		clientmock.CheckHeader().WithJSONHeaders().With(APIKeyHeader, "secret"))
}
...
func TestClient_UpdateRecords(t *testing.T) {
	client := mockBuilder().
		Route("PATCH /api/v1/servers/localhost/zones/example.org.",
			clientmock.ResponseFromFixture("zone.json"),
			clientmock.CheckRequestJSONBodyFromFile("zone-request.json")).
		Build(t)
...
func TestClient_getAPIVersion(t *testing.T) {
	client := mockBuilder().
		Route("GET /api",
			clientmock.ResponseFromFixture("versions.json")).
		Build(t)
