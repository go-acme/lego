...
func mockBuilder() *stubrouter.Builder[*Client] {
	return stubrouter.NewBuilder[*Client](
		...
		stubrouter.CheckHeader().WithJSONHeaders().With(APIKeyHeader, "secret"))
}
...
func TestClient_UpdateRecords(t *testing.T) {
	client := mockBuilder().
		Route("PATCH /api/v1/servers/localhost/zones/example.org.",
			stubrouter.ResponseFromFixture("zone.json"),
			stubrouter.CheckRequestJSONBodyFromFile("zone-request.json")).
		Build(t)
...
func TestClient_getAPIVersion(t *testing.T) {
	client := mockBuilder().
		Route("GET /api",
			stubrouter.ResponseFromFixture("versions.json")).
		Build(t)
