// Package poweradmin implements a DNS provider for solving the DNS-01 challenge using Poweradmin.
package poweradmin
...
// NewDNSProvider returns a DNSProvider instance configured for Poweradmin.
func NewDNSProvider() (*DNSProvider, error) {
	values, err := env.Get(EnvBaseURL, EnvAPIKey)
	if err != nil {
		return nil, fmt.Errorf("poweradmin: %w", err)
	}
...
// NewDNSProviderConfig return a DNSProvider instance configured for Poweradmin.
func NewDNSProviderConfig(config *Config) (*DNSProvider, error) {
	if config == nil {
		return nil, errors.New("poweradmin: the configuration of the DNS provider is nil")
	}
...
func (d *DNSProvider) Present(ctx context.Context, domain, token, keyAuth string) error {
	...
	record, err := d.client.CreateRecord(ctx, zone.ID, r)
	if err != nil {
		return fmt.Errorf("poweradmin: create record: %w", err)
	}
...
func (d *DNSProvider) CleanUp(ctx context.Context, domain, token, keyAuth string) error {
	...
	if !ok {
		return fmt.Errorf("poweradmin: unknown zone ID for '%s' '%s'", info.EffectiveFQDN, token)
	}
	...
	if !ok {
		return fmt.Errorf("poweradmin: unknown record ID for '%s' '%s'", info.EffectiveFQDN, token)
	}

	err := d.client.DeleteRecord(ctx, zoneID, recordID)
	if err != nil {
		return fmt.Errorf("poweradmin: delete record: %w", err)
	}
