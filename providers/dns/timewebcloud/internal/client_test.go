...
func TestClient_CreateRecord(t *testing.T) {
	client := mockBuilder().
		Route("POST /v1/domains/example.com/dns-records",
			clientmock.ResponseFromFixture("createDomainDNSRecord.json"),
			clientmock.CheckRequestJSONBody(`{"type":"TXT","value":"w6uP8Tcg6K2QR905Rms8iXTlksL6OD1KOWBxTK7wxPI","subdomain":"_acme-challenge"}`)).
		Build(t)
...
func TestClient_DeleteRecord_error(t *testing.T) {
	client := mockBuilder().
		Route("DELETE /v1/domains/example.com/dns-records/123",
			clientmock.ResponseFromFixture("error_unauthorized.json").
				WithStatusCode(http.StatusBadRequest)).
		Build(t)
