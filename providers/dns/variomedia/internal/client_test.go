...
func TestClient_CreateDNSRecord(t *testing.T) {
	client := mockBuilder().
		Route("POST /dns-records",
			servermock2.ResponseFromFixture("POST_dns-records.json"),
			servermock2.CheckHeader().
				WithContentType("application/vnd.api+json"),
			servermock2.CheckRequestJSONBody(`{"data":{"type":"dns-record","attributes":{"record_type":"TXT","name":"_acme-challenge","domain":"example.com","data":"test","ttl":300}}}`)).
		Build(t)
...
func TestClient_DeleteDNSRecord(t *testing.T) {
	client := mockBuilder().
		Route("DELETE /dns-records/test",
			servermock2.ResponseFromFixture("DELETE_dns-records_pending.json")).
		Build(t)
...
func TestClient_GetJob(t *testing.T) {
	client := mockBuilder().
		Route("GET /queue-jobs/test",
			servermock2.ResponseFromFixture("GET_queue-jobs.json")).
		Build(t)
