...
func TestDNSProvider_Present(t *testing.T) {
	provider := mockBuilder().
		Route("POST /dns-records",
			servermock.ResponseFromInternal("POST_dns_records.json"),
			servermock.CheckRequestJSONBodyFromInternal("POST_dns_records-request.json"),
			servermock.CheckHeader().
				WithContentType("application/vnd.api+json"),
		).
		Route("GET /queue-jobs/18181818",
			servermock.ResponseFromInternal("GET_queue_jobs.json"),
		).
		Build(t)
...
func TestDNSProvider_CleanUp(t *testing.T) {
	provider := mockBuilder().
		Route("DELETE /dns-records/18181818",
			servermock.ResponseFromInternal("DELETE_dns_records_pending.json"),
		).
		Route("GET /queue-jobs/303030",
			servermock.ResponseFromInternal("GET_queue_jobs.json"),
		).
		Build(t)

	provider.recordIDs["abc"] = "18181818"
